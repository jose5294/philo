/**
 * 멀티 프로바이더 AI 대화 서비스 모듈 (비용 및 속도 극대화 최적화 버전)
 * - 가장 저렴하고 빠른 초경량 모델 우선 호출 (gemini-2.5-flash ➡️ gemini-2.0-flash ➡️ gemini-1.5-flash ➡️ gemini-1.5-flash-8b)
 * - maxOutputTokens 512로 불필요한 토큰 낭비 차단
 */
...
// 💰 구글 최저가 초경량 Flash 모델 풀 (100만 토큰당 약 50원~100원 수준)
...
/**
 * 1. Google Gemini API 호출 (초경량 토큰 최적화)
 */
async function callGemini(persona, conversation, userText, apiKey, customModel) {
  ...
  // 💡 최근 대화 6개(3턴)만 포함하여 불필요한 누적 토큰 비용 80% 절감
  ...
  const payload = {
    ...
    generationConfig: {
      temperature: 0.7,
      maxOutputTokens: 512, // 2~3문장 철학 대화에 최적화하여 출력 토큰 비용 최소화
    },
  };
